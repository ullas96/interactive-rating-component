...
function App() {
  ...
  const Button = ({ number }) => {
    ...
    className=" btn-number bg-gray-700 h-10 w-10 rounded-full pt-1 text-gray-200 focus:bg-gray-400 transition-all duration-150">{number}</button>
  )}
  return (
    <>
    {!isSubmitted && (
    ...
    className="btn-rating w-full txt-white uppercase tracking-wide pt-3 pb-2 rounded-full">submit</button>
    </div>
    </div>
    )}
...
const ThankYou = ({ items, setIsSubmitted }) => {
  return (
    <div className="wrapper">
      ...
      <p className="p-rating bg-gray-700 rounde-full text-center text-sm px-4 pt-1 ">You selected {items} out of 5</p>
      </div>
  ...
  don’t hesitate to get in touch!</p>
      <div className="text-center">
        ...
        className="btn-rating w-full txt-white uppercase tracking-wide pt-3 pb-2 rounded-full">Rate again</button>
      </div>
    </div>
  )
}
